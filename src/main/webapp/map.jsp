...
function searchTouristSpots() {
    ...
    if(areaCode == "none"){
    	//url = `http://localhost:8080/trip/map?action=allArea`;
    	alert("지역을 입력해주세요!");
    	return;
    } else {
    	if(contentTypeId=="none"){
    		url = `http://localhost:8080/trip/map?action=all&sido_code=\${areaCode}`;
    	}
    	else{
    		url = `http://localhost:8080/trip/map?action=content&sido_code=\${areaCode}&content_type_id=\${contentTypeId}`;
    	}
    }
...
function displaySearchResults(spots) {
    ...
    spots.forEach((spot, index) => {
        var resultItem = document.createElement('div');
        resultItem.className = 'result-item';
        var imageSrc = spot.first_image ? spot.first_image : 'img/main2.png'; // 이미지가 없으면 기본 이미지로 설정
        resultItem.innerHTML = `
            <div class="info-container">
                <div class="title">\${spot.title}</div>
                <div class="description">\${spot.addr1} \${spot.addr2 ? spot.addr2 : ''}</div>
...
function displayTouristSpots(spots) {
    ...
    spots.forEach(spot => {
        ...
        kakao.maps.event.addListener(marker, 'click', function() {
            ...
            // 이미지가 없는 경우 대체 이미지 경로 설정
            var imageToShow = spot.first_image ? spot.first_image : 'img/main2.png';
